...
const AdminAccounts = () => {
  ...
  const handleDelete = async (userId) => {
    const confirmed = window.confirm("❗ Are you sure you want to delete this user and their appointments?");
    if (!confirmed) return;
    ...
    try {
      ...
      console.error("❌ Failed to delete user via Cloud Function:", error.message);
              ...
              {expandedId === user.id && (
                <div style={{ marginTop: "12px", fontSize: "0.95rem", lineHeight: "1.5" }}>
                  ...
                  <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(user.id); }} style={btn("#e60000")}>🗑️ Delete</button>
                  </div>
                </div>
              )}
            </div>
